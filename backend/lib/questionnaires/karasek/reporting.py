...
class KarasekReporting:
    ...
    def generate_word_report(self, df: pd.DataFrame, metrics: Dict, output_dir: str, figures: Dict = None) -> str:
        figures = figures or {}
        doc = Document()
        ...
        quadrant_order = ["Actif", "Detendu", "Tendu", "Passif"]
        ...
        self._add_section_title(doc, "3. Visualisations", level=1)
        
        if figures.get("scatter_theoretical"):
            doc.add_heading("3.1 Grille de Karasek", level=2)
            doc.add_picture(figures["scatter_theoretical"], width=Inches(6))
        
        if figures.get("heatmap_karasek"):
            doc.add_heading("3.2 Répartition par Direction", level=2)
            doc.add_picture(figures["heatmap_karasek"], width=Inches(7))
        
        if figures.get("age_pyramid"):
            doc.add_heading("3.3 Pyramide des âges", level=2)
            doc.add_picture(figures["age_pyramid"], width=Inches(5))
