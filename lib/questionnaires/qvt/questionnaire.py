...
def _fuzzy_rename(df: pd.DataFrame) -> pd.DataFrame:
    """Renomme les colonnes source vers les codes internes QVT via matching normalisé."""
...
class QVTQuestionnaire(BaseQuestionnaire):
    ...
    def clean(self, df: pd.DataFrame) -> pd.DataFrame:
        # Nettoyage générique partagé par l'ensemble des questionnaires.
        df, _ = clean_pii(df)
    ...
    def score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Nettoyage Likert : on force les réponses dans l'échelle attendue.
        for col in [c for c in df_out.columns if ITEM_PATTERN.match(c)]:
            df_out[col] = clip_likert(df_out[col], LIKERT_MIN, LIKERT_MAX)

        # Chaque dimension QVT agrège un groupe d'items homogènes.
        ...
        # Le score global est une moyenne des dimensions disponibles.
    ...
    def classify(self, df: pd.DataFrame) -> pd.DataFrame:
        df_out = df.copy()
        # Chaque score numérique est converti en catégorie lisible pour
